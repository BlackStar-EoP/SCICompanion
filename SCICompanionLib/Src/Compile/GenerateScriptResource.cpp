...
//            property_vector::const_iterator overriddenIt = find_if(newProps.begin(), newProps.end(), bind2nd(MatchSelector(), speciesProp.wSelector));
            ...
            property_vector::const_iterator overriddenIt = newProps.begin();
            for (; overriddenIt != newProps.end(); ++overriddenIt)
            {
                if (overriddenIt->wSelector == speciesProp.wSelector)
                    break;
            }
        ...
        for (species_property &newProp : newProps)
        {
            ...
            property_vector::const_iterator speciesIt = newProps.begin();
            for (; speciesIt != newProps.end(); ++speciesIt)
            {
                if (speciesIt->wSelector == newProp.wSelector)
                    break;
            }
